...
class Websocket implements \Workerman\Protocols\ProtocolInterface
{
	...
	private static function errorparse($connection, $buffer)
	{
		if($connection->onError != null){
			self::parseHttpHeader($buffer);
			try
			{
				return call_user_func($connection->onError, $connection, $buffer);
			}
			catch(\Exception $e)
			{
				echo $e;
