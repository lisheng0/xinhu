...
$(document).ready(function(){
	...
	var a = $('#view_{rand}').bootstable({
		...
		{
			text:'生成二维码',dataIndex:'qrcode',renderer:function(v,d,oi){
				return '<div id="qr'+d.id+'" onclick="setQrcode('+d.id+')"><a href="javascript:;" >点击生成二维码</a></div>';
			}	
		}],
...
<table width="100%"><tr>
	...
	<td align="right" nowrap>
		...
		<button class="btn btn-info" id="edit_{rand}" click="clickwin,1" type="button"><i class="icon-edit"></i> 编辑 </button>
		
	</td>
	<div style="display:none" id="rr" >111</div>
<script type="text/javascript">
function setQrcode(id){
	var url = self.location.href;
	var url = url.split('?');
	var url = url[0]+'/'+'task.php?a=p&num=goods&mid='+id;
	//判断是否已生成二维码
	if($("#qr"+id).find("canvas").length == 0){
		//生成二维码
		$("#qr"+id).qrcode({
			render: "canvas", //也可以替换为
			width: 100,
			height: 100,
			text: url,
		});
		...
		$("#qr"+id).find("canvas").remove();
		$("#qr"+id).find("a").text('点击生成二维码');
	}
}
</script>
</tr></table>
